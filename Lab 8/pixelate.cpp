...
void increasetwotimes(int image[MAX_H][MAX_W], int& h, int& w) 
{
    int twotimes[MAX_H][MAX_W];
    for(int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {

            twotimes[i*2][j*2] = image[i][j];

            twotimes[(i*2) + 1][j * 2] = image[i][j];

            twotimes[i*2][(j*2) + 1] = image[i][j];

            twotimes[(i*2) + 1][(j*2) + 1] = image[i][j];
        }
    }
    
    writeImage("outImage.pgm", twotimes, h * 2, w * 2);
}
...
void pixelate(int image[MAX_H][MAX_W], int& h, int& w)
{
    ...
    for(int i = 0; i < h; i = i + 2) {
        for (int j = 0; j < w; j = j + 2) {
            int blur = (((image[i][j]) + (image[i][j + 1]) + (image[i + 1][j]) + (image[i + 1][j + 1]))/4);
            pix[i][j] = blur;
            pix[i][j + 1] = blur;
            pix[i + 1][j] = blur;
            pix[i + 1][j + 1] = blur;
    ...
    writeImage("outImage.pgm", pix, h, w);
}
